...
int battery_status_read() {
    ...
    ESP_LOGI(TAG, "ADC voltage: %d Battery voltage: %d", adc_voltage, voltage);
    //gpio_set_level(GPIO_BAT_ADC_EN, 0);
    return voltage;
}

void send_battery_status(int voltage) {
    battery_status_event_t* battery_status = (battery_status_event_t*) cc_get_buffer();
    battery_status->voltage = voltage;
    cc_send_data(PROTOCOL_OUT_BATTERY_STATUS, (uint8_t*) battery_status, sizeof(battery_status_event_t));
}